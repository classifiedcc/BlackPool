...
doc:
  cargo doc --package blackpool --open
...
proxy-mainnet:
  cargo run --features reload -- \
    ...
    --start-diff 0.001 \
    --http-port 8081 \
    --upstream bc1qyr294wemhvcp69dheccp2nat2yemtxfd6sc96e.proxy@blackpool.com:42069
...
publish-release revision='master':
  ...
  set -euxo pipefail
  rm -rf tmp/release
  git clone git@github.com:classifiedcc/blackpool.git tmp/release
  cd tmp/release
  git checkout {{ revision }}
  cargo publish
  cd ../..

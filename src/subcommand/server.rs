...
mod tests {
    ...
    #[test]
    fn default_address() {
        let config = parse_server_config("blackpool server");
        assert_eq!(config.address(), "0.0.0.0");
    }

    #[test]
    fn override_address() {
        let config = parse_server_config("blackpool server --address 127.0.0.1");
    ...
    fn override_acme_cache_via_data_dir() {
        let config = parse_server_config("blackpool server --data-dir /custom/path");
        assert_eq!(
            config.acme_cache(),
    ...
    fn override_acme_domains() {
        let config =
            parse_server_config("blackpool server --acme-domain example.com --acme-domain foo.bar");
        assert_eq!(
    ...
    fn override_acme_contacts() {
        let config = parse_server_config("blackpool server --acme-contact admin@example.com");
        assert_eq!(
    ...
    #[test]
    fn admin_token() {
        let config = parse_server_config("blackpool server --admin-token verysecrettoken");
        assert_eq!(config.admin_token(), Some("verysecrettoken"));
    }

    #[test]
    fn default_domain() {
        let config = parse_server_config("blackpool server --acme-domain example.com");
    ...
    fn override_domains_no_fallback() {
        let config = parse_server_config("blackpool server --acme-domain custom.domain");
        let domains = config.domains().unwrap();
    ...
    fn override_data_dir() {
        let config = parse_server_config("blackpool server --data-dir /var/pool");
    ...
    fn override_database_url() {
        let config = parse_server_config("blackpool server --database-url postgres://user:pass@host/db");
    ...
    #[test]
    fn override_log_dir() {
        let config = parse_server_config("blackpool server --log-dir /logs");
    ...
    #[test]
    fn override_port() {
        let config = parse_server_config("blackpool server --port 8080");
    ...
    fn override_nodes_single_http() {
        let config = parse_server_config("blackpool server --nodes http://localhost:80");
    ...
    fn override_nodes_single_https() {
        let config = parse_server_config("blackpool server --nodes https://blackpool.com");
        let expected = vec![Url::parse("https://blackpool.com").unwrap()];
        assert_eq!(config.nodes(), expected);
    }
    ...
    fn multiple_nodes() {
        let config = parse_server_config(
            "blackpool server --nodes http://localhost:80 --nodes https://blackpool.com",
        );
        let expected = vec![
            Url::parse("http://localhost:80").unwrap(),
            Url::parse("https://blackpool.com").unwrap(),
        ];
        assert_eq!(config.nodes(), expected);
    }
    ...
    fn invalid_node_url() {
        parse_server_config("blackpool server --nodes invalid_url");
    }

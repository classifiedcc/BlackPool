...
impl Settings {
    ...
    pub(crate) async fn bitcoin_rpc_client(&self) -> Result<BitcoindClient> {
        ...
        if rpc_chain != para_chain {
            bail!("Bitcoin RPC server is on {rpc_chain} but blackpool is on {para_chain}");
        }
...
mod tests {
    ...
    fn parse_router_options(args: &str) -> RouterOptions {
        match Arguments::try_parse_from(args.split_whitespace()) {
            Ok(arguments) => match arguments.subcommand {
                crate::subcommand::Subcommand::Router(router) => router.options,
                subcommand => panic!("unexpected subcommand: {subcommand:?}"),
            },
            Err(err) => panic!("error parsing arguments: {err}"),
        }
    ...
    fn pool_defaults_are_sane() {
        let options = parse_pool_options("blackpool pool");
    ...
    fn pool_override_address_and_port() {
        let options = parse_pool_options("blackpool pool --address 127.0.0.1 --port 9999");
        let settings = Settings::from_pool_options(options).unwrap();
    ...
    fn pool_override_chain_changes_default_rpc_port() {
        let options = parse_pool_options("blackpool pool --chain signet");
        let settings = Settings::from_pool_options(options).unwrap();
    ...
    fn pool_explicit_bitcoin_rpc_port_wins() {
        let options = parse_pool_options("blackpool pool --chain regtest --bitcoin-rpc-port 4242");
        let settings = Settings::from_pool_options(options).unwrap();
    ...
    fn pool_override_version_mask() {
        let options = parse_pool_options("blackpool pool --version-mask 00fff000");
        let settings = Settings::from_pool_options(options).unwrap();
    ...
    fn pool_credentials_cookiefile_when_no_creds() {
        let options =
            parse_pool_options("blackpool pool --bitcoin-rpc-cookie-file /var/lib/bitcoind/.cookie");
        let settings = Settings::from_pool_options(options).unwrap();
    ...
    fn pool_cookie_file_from_explicit_cookie_path() {
        let options = parse_pool_options("blackpool pool --bitcoin-rpc-cookie-file /x/y/.cookie");
        let settings = Settings::from_pool_options(options).unwrap();
    ...
    fn pool_cookie_file_from_bitcoin_data_dir_and_chain() {
        let options =
            parse_pool_options("blackpool pool --bitcoin-data-dir /data/bitcoin --chain regtest");
        let settings = Settings::from_pool_options(options).unwrap();
        ...
        let options =
            parse_pool_options("blackpool pool --bitcoin-data-dir /data/bitcoin --chain signet");
        let settings = Settings::from_pool_options(options).unwrap();
        ...
        let options =
            parse_pool_options("blackpool pool --bitcoin-data-dir /data/bitcoin --chain mainnet");
        let settings = Settings::from_pool_options(options).unwrap();
    ...
    fn pool_rpc_url_reflects_port_choice() {
        let options = parse_pool_options("blackpool pool --bitcoin-rpc-port 12345");
        let settings = Settings::from_pool_options(options).unwrap();
    ...
    fn pool_zmq_block_notifications() {
        let options = parse_pool_options("blackpool pool --zmq-block-notifications tcp://127.0.0.1:69");
        let settings = Settings::from_pool_options(options).unwrap();
    ...
    #[test]
    fn pool_start_diff() {
        let options = parse_pool_options("blackpool pool --start-diff 0.00001");
        let settings = Settings::from_pool_options(options).unwrap();
        assert_eq!(settings.start_diff, Difficulty::from(0.00001));

        let options = parse_pool_options("blackpool pool --start-diff 111");
        let settings = Settings::from_pool_options(options).unwrap();
    ...
    fn pool_vardiff_period() {
        let options = parse_pool_options("blackpool pool --vardiff-period 10.0");
        let settings = Settings::from_pool_options(options).unwrap();
        assert_eq!(settings.vardiff_period, Duration::from_secs(10));

        let options = parse_pool_options("blackpool pool --vardiff-period 0.5");
        let settings = Settings::from_pool_options(options).unwrap();
    ...
    fn pool_vardiff_window() {
        let options = parse_pool_options("blackpool pool --vardiff-window 60");
        let settings = Settings::from_pool_options(options).unwrap();
        assert_eq!(settings.vardiff_window, Duration::from_secs(60));

        let options = parse_pool_options("blackpool pool --vardiff-window 600.5");
        let settings = Settings::from_pool_options(options).unwrap();
    ...
    fn pool_enonce1_size_override() {
        let options = parse_pool_options("blackpool pool --enonce1-size 6");
        let settings = Settings::from_pool_options(options).unwrap();
    ...
    fn pool_enonce1_size_boundaries() {
        let options = parse_pool_options("blackpool pool --enonce1-size 2");
        let settings = Settings::from_pool_options(options).unwrap();
        assert_eq!(settings.enonce1_size, 2);

        let options = parse_pool_options("blackpool pool --enonce1-size 8");
        let settings = Settings::from_pool_options(options).unwrap();
    ...
    fn pool_enonce1_size_too_small() {
        let options = parse_pool_options("blackpool pool --enonce1-size 1");
    ...
    fn pool_enonce1_size_too_large() {
        let options = parse_pool_options("blackpool pool --enonce1-size 9");
        let err = Settings::from_pool_options(options).unwrap_err();
    ...
    fn pool_enonce2_size_override() {
        let options = parse_pool_options("blackpool pool --enonce2-size 4");
        let settings = Settings::from_pool_options(options).unwrap();
    ...
    fn pool_enonce2_size_boundaries() {
        let options = parse_pool_options("blackpool pool --enonce2-size 2");
        let settings = Settings::from_pool_options(options).unwrap();
        assert_eq!(settings.enonce2_size, 2);

        let options = parse_pool_options("blackpool pool --enonce2-size 8");
        let settings = Settings::from_pool_options(options).unwrap();
    ...
    fn pool_enonce2_size_too_small() {
        let options = parse_pool_options("blackpool pool --enonce2-size 1");
        let err = Settings::from_pool_options(options).unwrap_err();
    ...
    fn pool_enonce2_size_too_large() {
        let options = parse_pool_options("blackpool pool --enonce2-size 9");
        let err = Settings::from_pool_options(options).unwrap_err();
    ...
    fn pool_min_diff_parsing() {
        let options = parse_pool_options("blackpool pool --min-diff 0.001");
        let settings = Settings::from_pool_options(options).unwrap();
    ...
    fn pool_max_diff_parsing() {
        let options = parse_pool_options("blackpool pool --max-diff 1000");
        let settings = Settings::from_pool_options(options).unwrap();
    ...
    fn pool_valid_min_max_diff_config() {
        let options = parse_pool_options("blackpool pool --start-diff 10 --min-diff 1 --max-diff 100");
        assert!(Settings::from_pool_options(options).is_ok());
    }

    #[test]
    fn pool_start_diff_below_min_diff_fails() {
        let options = parse_pool_options("blackpool pool --start-diff 1 --min-diff 10");
        assert!(Settings::from_pool_options(options).is_err());
    }

    #[test]
    fn pool_start_diff_above_max_diff_fails() {
        let options = parse_pool_options("blackpool pool --start-diff 100 --max-diff 10");
        assert!(Settings::from_pool_options(options).is_err());
    }

    #[test]
    fn pool_min_diff_above_max_diff_fails() {
        let options = parse_pool_options("blackpool pool --start-diff 50 --min-diff 100 --max-diff 10");
        assert!(Settings::from_pool_options(options).is_err());
    }
    ...
    fn acme_options() {
        ...
        case("blackpool pool", "blackpool proxy --upstream bar@foo:1234", |s| {
        ...
        case(
            "blackpool pool --acme-domain foo.bar --acme-domain baz.qux",
            "blackpool proxy --upstream bar@foo:1234 --acme-domain foo.bar --acme-domain baz.qux",
            |s| {
        ...
        case(
            "blackpool pool --acme-contact foo@bar --acme-contact baz@qux",
            "blackpool proxy --upstream bar@foo:1234 --acme-contact foo@bar --acme-contact baz@qux",
            |s| {
        ...
        case(
            "blackpool pool --acme-cache /foo/bar",
            "blackpool proxy --upstream bar@foo:1234 --acme-cache /foo/bar",
            |s| {
        ...
        case(
            "blackpool pool --data-dir /foo",
            "blackpool proxy --upstream bar@foo:1234 --data-dir /foo",
            |s| {
    ...
    fn proxy_defaults_are_sane() {
        let options = parse_proxy_options("blackpool proxy --upstream bar@foo:1234");
    ...
    fn proxy_override_address_and_port() {
        let options = parse_proxy_options(
            "blackpool proxy --upstream bar@foo:1234 --address 127.0.0.1 --port 9999",
        );
        let settings = Settings::from_proxy_options(options).unwrap();
    ...
    fn proxy_override_http_port() {
        let options = parse_proxy_options("blackpool proxy --upstream bar@foo:1234 --http-port 8080");
        let settings = Settings::from_proxy_options(options).unwrap();
    ...
    fn proxy_override_timeout() {
        let options = parse_proxy_options("blackpool proxy --upstream bar@foo:1234 --timeout 60");
        let settings = Settings::from_proxy_options(options).unwrap();
    ...
    fn proxy_password_override() {
        let options = parse_proxy_options("blackpool proxy --upstream bar:baz@foo:1234");
        let settings = Settings::from_proxy_options(options).unwrap();
    ...
    fn proxy_enonce1_extension_size_override() {
        let options =
            parse_proxy_options("blackpool proxy --upstream bar@foo:1234 --enonce1-extension-size 3");
        let settings = Settings::from_proxy_options(options).unwrap();
    ...
    fn proxy_enonce1_extension_size_boundaries() {
        let options =
            parse_proxy_options("blackpool proxy --upstream bar@foo:1234 --enonce1-extension-size 1");
        let settings = Settings::from_proxy_options(options).unwrap();
        assert_eq!(settings.enonce1_extension_size, 1);

        let options =
            parse_proxy_options("blackpool proxy --upstream bar@foo:1234 --enonce1-extension-size 6");
        let settings = Settings::from_proxy_options(options).unwrap();
    ...
    fn proxy_enonce1_extension_size_too_small() {
        let options =
            parse_proxy_options("blackpool proxy --upstream bar@foo:1234 --enonce1-extension-size 0");
    ...
    fn proxy_enonce1_extension_size_too_large() {
        let options =
            parse_proxy_options("blackpool proxy --upstream bar@foo:1234 --enonce1-extension-size 7");
        let err = Settings::from_proxy_options(options).unwrap_err();
        assert!(
            err.to_string()
    ...
    fn high_diff_port() {
        ...
        case("blackpool pool", "blackpool proxy --upstream bar@foo:1234", None);

        case(
            "blackpool pool --high-diff-port 3333",
            "blackpool proxy --upstream bar@foo:1234 --high-diff-port 3333",
            Some(3333),
        );
    }
    ...
    fn high_diff_port_equals_port_fails() {
        let pool = parse_pool_options("blackpool pool --port 3333 --high-diff-port 3333");
        let proxy = parse_proxy_options(
            "blackpool proxy --upstream bar@foo:1234 --port 3333 --high-diff-port 3333",
        );
        assert!(
    ...
    fn high_diff_port_equals_http_port_fails() {
        let pool = parse_pool_options("blackpool pool --http-port 8080 --high-diff-port 8080");
        let proxy = parse_proxy_options(
            "blackpool proxy --upstream bar@foo:1234 --http-port 8080 --high-diff-port 8080",
        );
        assert!(
            Settings::from_pool_options(pool)
                .unwrap_err()
    ...
    fn high_diff_port_above_max_diff_fails() {
        let pool = parse_pool_options("blackpool pool --high-diff-port 3333 --max-diff 100");
        let proxy = parse_proxy_options(
            "blackpool proxy --upstream bar@foo:1234 --high-diff-port 3333 --max-diff 100",
        );
        assert!(
            Settings::from_pool_options(pool)
                .unwrap_err()
    ...
    fn high_diff_port_below_min_diff_fails() {
        let pool = parse_pool_options(
            "blackpool pool --high-diff-port 3333 --min-diff 2000000 --start-diff 2000000",
        );
        let proxy = parse_proxy_options(
            "blackpool proxy --upstream bar@foo:1234 --high-diff-port 3333 --min-diff 2000000 --start-diff 2000000",
        );
        assert!(
            Settings::from_pool_options(pool)
                .unwrap_err()
    ...
    fn shared_defaults_pool_proxy_and_router() {
        let pool = Settings::from_pool_options(parse_pool_options("blackpool pool")).unwrap();
        let proxy =
            Settings::from_proxy_options(parse_proxy_options("blackpool proxy --upstream bar@foo:1234"))
                .unwrap();
        let router = Settings::from_router_options(parse_router_options(
            "blackpool router --upstream bar@foo:1234",
        ))
        .unwrap();

        for settings in [&proxy, &router] {
            assert_eq!(pool.address, settings.address);
            assert_eq!(pool.port, settings.port);
            assert_eq!(pool.start_diff, settings.start_diff);
            assert_eq!(pool.vardiff_period, settings.vardiff_period);
            assert_eq!(pool.vardiff_window, settings.vardiff_window);
    ...
    fn settings_default_matches_pool_defaults() {
        ...
        let pool_settings = Settings::from_pool_options(parse_pool_options("blackpool pool")).unwrap();
    ...
    fn router_defaults_are_sane() {
        let options = parse_router_options("blackpool router --upstream bar@foo:1234");
    ...
    fn router_override_address_and_port() {
        let options = parse_router_options(
            "blackpool router --upstream bar@foo:1234 --address 127.0.0.1 --port 9999",
        );
        let settings = Settings::from_router_options(options).unwrap();
    ...
    fn router_override_timeout() {
        let options = parse_router_options("blackpool router --upstream bar@foo:1234 --timeout 60");
        let settings = Settings::from_router_options(options).unwrap();
    ...
    fn router_enonce1_extension_size_override() {
        let options =
            parse_router_options("blackpool router --upstream bar@foo:1234 --enonce1-extension-size 3");
        let settings = Settings::from_router_options(options).unwrap();
        assert_eq!(settings.enonce1_extension_size, 3);
    }
    ...
    fn router_enonce1_extension_size_boundaries() {
        let options =
            parse_router_options("blackpool router --upstream bar@foo:1234 --enonce1-extension-size 1");
        let settings = Settings::from_router_options(options).unwrap();
        assert_eq!(settings.enonce1_extension_size, 1);

        let options =
            parse_router_options("blackpool router --upstream bar@foo:1234 --enonce1-extension-size 6");
        let settings = Settings::from_router_options(options).unwrap();
        assert_eq!(settings.enonce1_extension_size, 6);
    }
    ...
    fn router_enonce1_extension_size_too_small() {
        let options =
            parse_router_options("blackpool router --upstream bar@foo:1234 --enonce1-extension-size 0");
    ...
    fn router_enonce1_extension_size_too_large() {
        let options =
            parse_router_options("blackpool router --upstream bar@foo:1234 --enonce1-extension-size 7");
        let err = Settings::from_router_options(options).unwrap_err();
        assert!(
            err.to_string()
                .contains("enonce1_extension_size (7) must be <=")
